...
bool tietokantaMoottori::nostaRahaa(QString tilinumero, double nosto)
{
    ...
    else
    {
        ...
        tiedustelu.bindValue(":tilinumero", tilinumero);
        tiedustelu.exec();
        tiedustelu.prepare("INSERT INTO tapahtumatTable (tilinumeroKaksi,pvm,tapahtuma,summa) VALUES(:tilinumero,CURDATE(),'Nosto',-:nosto)");
        tiedustelu.bindValue(":tilinumero", tilinumero);
        tiedustelu.bindValue(":nosto", nosto);
        tiedustelu.bindValue(":tilinumeroKaksi", tilinumero);
        tiedustelu.exec();
        return true;
    }
}
